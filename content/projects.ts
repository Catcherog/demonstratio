...
export const projects: Project[] = [
  ...
  },
  {
    ...
    provisional: true,
    evidenceLabel:
      "公开演示域名 chat.jael.com 当前连接关闭,演示维护中。自动应答率与准确率为内部小样本估算,待固定评测集与线上数据验证。",
    metrics: [
      { value: "18", label: "类咨询场景" },
      { value: "3 级", label: "置信度分流" },
      { value: "LangGraph", label: "工作流编排" },
      { value: "28", label: "个测试文件" },
    ],
    ...
    productStrategy: [
      "产品目标:用 LangGraph 编排检索、风险分流、生成、质量检查与人工接管,完成 Web/API 端到端 MVP。",
      "产品边界:当前为业务验证阶段,公开演示维护中,不主张生产 SLO。",
      "方案选择:采用确定性节点与 LLM 节点组合工作流,保留飞书权威主源和本地 JSON 镜像。",
      "暂不做:不追求无条件自动回复,不把内部小样本估算写成生产指标。",
    ],
    decisions: [
    ...
    outcomes: [
      ...
      "LangGraph 工作流、RAG 检索、风险分流、人工接管与反馈闭环均有代码证据。",
      "819 tracked 文件、28 个测试文件、560 个文档支持当前架构。",
    ],
    architecture: [
    ...
    keyWorkflow: [
      { label: "1. 意图识别", detail: "用户咨询 → 场景路由(18 类)→ 敏感边界判断" },
      { label: "2. 查询改写", detail: "对话上下文 + 关键词补全 → 检索查询" },
      { label: "3. 检索重排", detail: "向量召回 → 候选重排序 → 引用上下文" },
      { label: "4. 置信度分流", detail: "高分直接建议 / 中分人工复核 / 低分转人工" },
      { label: "5. 反馈回流", detail: "归档 → 清洗 → 人工确认 → 知识同步" },
    ],
    tradeoffs: [
    ...
    evidenceLinks: [
      { label: "LangGraph 流程图", type: "architecture", ref: "SCS-001" },
      { label: "客服聊天页面", type: "screenshot", ref: "SCS-002" },
      { label: "人工接管案例", type: "screenshot", ref: "SCS-003" },
      { label: "知识检索证据", type: "api", ref: "SCS-004" },
      { label: "28 个测试文件", type: "test", ref: "SCS-005" },
      { label: "反馈闭环代码", type: "api", ref: "SCS-006" },
      { label: "Demo Disclosure", type: "deploy", ref: "SCS-007" },
    ],
    myContribution: [
