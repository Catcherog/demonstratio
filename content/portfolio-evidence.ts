...
export const portfolioEvidence: PortfolioEvidence[] = [
  ...
  buildEvidence(lumenInk, {
    ...
    summary: "光砚工作台中的模型 Provider、生成模式与参数设置界面；在 AI BUSINESS OS 集成层，服务端 RunningHub adapter 承接 workflow engine，浏览器只看到能力契约和状态。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-LUMEN-SOURCE", "E-LUMEN-EDIT"],
    scope: "Workbench → Task → Provider adapter → Result/history",
    boundary: "RunningHub credentials/config remain server-only；缺少它们时 connected path 为 BLOCKED。图示只说明工作台与 adapter 边界，不是 RunningHub LIVE 证明，也不代表所有 Provider 或编辑模式均完成同等验证。",
    assetUrl: "/evidence/lumen/02-provider-routing.png",
    tags: ["光砚", "provider", "RunningHub", "任务", "架构", "恢复"],
