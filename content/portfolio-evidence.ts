...
export const portfolioEvidence: PortfolioEvidence[] = [
  ...
  buildEvidence(dataPlatform, {
    ...
    kind: "interactive",
    title: "飞书智能录入台｜公开 Portal",
    summary:
      "公开前端展示来源输入、候选确认、SOP 治理、写入计划和结果回读等产品交互，用于说明受控摄入入口的产品形态。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-FEISHU-SOURCES", "E-FEISHU-PILOT-TESTS"],
    scope: "公开 Portal UI 与受控摄入流程展示",
    boundary:
      "Portal 前端可公开访问；Collator、SOP 和正式业务 Base 写入仍依赖受控环境。本轮真实运行结果为 partial，不作为成功写入案例，也不代表生产业务闭环已经开放。",
    href: "https://portal-seven-jade-47.vercel.app/",
    verifiedAt: "2026-08-03",
    tags: ["飞书", "portal", "智能录入台", "candidate", "sop", "受控写入"],
    roleWeights: { recruiter: 9, "product-lead": 9, technical: 6 },
  }),
  buildEvidence(dataPlatform, {
    id: "data-platform-write-proof",
    kind: "video",
    title: "飞书测试 Base 写入证据",
    summary: "展示 Candidate 通过治理门后进入测试 Base 的真实写入过程；用于公开说明测试写入链路已验证。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-FEISHU-PILOT-TESTS"],
    scope: "测试 Base 写入与精确清理",
    boundary: "仅测试 Base，不触达正式业务数据；字段级差异与正式写入仍 fail-closed。",
    assetUrl: "/evidence/data-platform/feishu-write-proof.mp4",
    verifiedAt: "2026-08-05",
    tags: ["飞书", "写入", "证据", "test base"],
    roleWeights: { recruiter: 8, "product-lead": 9, technical: 8 },
  }),
  buildEvidence(dataPlatform, {
    id: "data-platform-walkthrough",
    kind: "document",
    title: "真实验证范围与写入边界",
    summary: "当前公开证据聚焦于最关键的可闭合能力：测试 Base 摄入、SOP Gate、写入、幂等与精确清理已完成真实 E2E 验证。其余环节不以“待补素材”形式悬置，而是明确保持为未公开验证状态。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-FEISHU-PILOT-TESTS", "E-FEISHU-LIVE-SCHEMA"],
    scope: "测试 Base E2E · 生产只读 Schema 检查",
    boundary: "测试写入链路已验证，不代表生产业务 Pilot 已运行；字段级差异与正式业务写入仍 fail-closed。",
    href: "/experience/feishu-intake-demo",
    verifiedAt: "2026-08-05",
    tags: ["飞书", "验证边界", "写入", "SOP"],
    roleWeights: { recruiter: 6, "product-lead": 7, technical: 7 },
  }),
  ...
  buildEvidence(lumenInk, {
    ...
    assetUrl: "/evidence/lumen/01-workbench.png",
  ...
  buildEvidence(lumenInk, {
    ...
    kind: "image",
    title: "Provider 选择与参数编排",
    summary: "光砚工作台中的模型 Provider、生成模式与参数设置界面。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-LUMEN-SOURCE", "E-LUMEN-EDIT"],
    scope: "Workbench → Task → Provider adapter → Result/history",
    boundary: "图示区分已验证路径和未验证编辑模式，不代表所有 Provider 均完成同等验证。",
    assetUrl: "/evidence/lumen/02-provider-routing.png",
  ...
  buildEvidence(lumenInk, {
    ...
    kind: "image",
    title: "真实图像生成与编辑结果",
    summary: "光砚在真实 Provider 路径下完成图像生成或编辑后的结果界面。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-LUMEN-EDIT"],
    scope: "Seedream 4.5 文生图与图生图",
    boundary: "液化、修复、消除和其他模式仍未验证；不由本次结果外推。",
    assetUrl: "/evidence/lumen/03-edit-verification.png",
  ...
  buildEvidence(lumenInk, {
    ...
    kind: "image",
    title: "在线体验与完整结果预览",
    summary: "光砚在线工作台展示完整图片结果与编辑操作入口。",
    state: "available",
    publicSafe: true,
    ...
    assetUrl: "/evidence/lumen/04-live-entry.png",
  ...
  buildEvidence(lumenInk, {
    ...
    kind: "document",
    title: "真实验证范围与体验入口",
    summary: "当前公开证据聚焦于最关键的可闭合能力：Seedream 4.5 文生图与图生图已完成真实 Provider 路径验证。其余模式不以“待补素材”形式悬置，而是明确保持为未公开验证状态。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-LUMEN-SOURCE", "E-LUMEN-EDIT"],
    scope: "文生图 / 图生图 真实验证",
    boundary: "液化、修复、消除及其他编辑模式仍未验证；登录链路仍可能因 auth throttle 或 CloudBase 可达性 fail-closed 返回 503，因此不表达为全面可用。",
    href: "https://lumen-ink.vercel.app/",
    verifiedAt: "2026-08-05",
    tags: ["光砚", "验证边界", "seedream", "体验"],
