...
export const portfolioEvidence: PortfolioEvidence[] = [
  ...
  buildEvidence(serviceAgent, {
    ...
    kind: "interactive",
    title: "B1 / B2 / B3 受控演示",
    summary: "公开前端提供三个静态场景与可解释降级，避免在实时后端未接入时出现裸错误。",
    state: "available",
    publicSafe: true,
    evidenceRefs: ["E-SCS-PRODUCTION"],
    scope: "公开前端的静态受控体验",
    boundary: "前端仍指向 Render 静态降级，不调用 CloudBase Deploy 039。",
    href: "https://zehuai-customer-demo.vercel.app/",
    tags: ["agent", "demo", "b1", "b2", "b3", "体验"],
    roleWeights: { recruiter: 9, "product-lead": 7, technical: 5 },
  }),
  buildEvidence(serviceAgent, {
    id: "service-agent-live-frontend",
    kind: "interactive",
    title: "CloudBase 实时前端接入",
    summary: "计划将公开前端切换到已验证的 CloudBase 后端，并补齐切换后的端到端回归。",
    state: "planned",
    publicSafe: true,
    evidenceRefs: ["E-SCS-DEPLOY-039"],
    scope: "公开前端到 Deploy 039 的实时 API",
    boundary: "待补素材；前端尚未切换，不提供实时体验入口。",
    tags: ["agent", "frontend", "cloudbase", "planned"],
    roleWeights: { recruiter: 6, "product-lead": 6, technical: 7 },
  }),
  buildEvidence(serviceAgent, {
    id: "service-agent-walkthrough",
    kind: "video",
    title: "多轮检索与人工接管 Walkthrough",
    summary: "计划录制普通咨询、多轮省略、高风险咨询和知识缺口四条路径。",
    state: "planned",
    publicSafe: true,
    evidenceRefs: ["E-SCS-DEPLOY-039"],
    scope: "受控场景与状态追踪说明",
    boundary: "待补素材；未录制前不显示播放器或播放按钮。",
    transcript: "计划内容：B1 支持回答、多轮查询解析、B2 高风险人工接管、B3 证据不足转人工。",
    tags: ["agent", "视频", "多轮", "handoff", "planned"],
    roleWeights: { recruiter: 5, "product-lead": 5, technical: 5 },
  }),
