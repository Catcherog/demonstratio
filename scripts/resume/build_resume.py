...
Online Beta 候选｜界面可访问，后端 / 核心编辑链路待在线复核；采用 BYO Key 使用边界。
...
REQUIRED = [
    ...
    "Public Demo 候选", "备用证据可用", "Provider 抽象", "项目 / 任务 / 结果持久化与恢复",
    "_id 更新缺陷已修复", "本地 client/server 回归与 build 通过", "Online Beta 候选",
    "后端 / 核心编辑链路待在线复核", "BYO Key",
]
FORBIDDEN = [
...
def build_docx(photo_source: Path) -> None:
    ...
    add_project(doc, "Service Agent", "RAG 客服与知识飞轮", [
        ...
        ("公网状态：", "Public Demo 候选｜公网恢复中 / 备用证据可用。"),
    ])
    ...
    add_project(doc, "光砚 Lumen", "AI 图像编辑工作台", [
        ...
        ("状态与边界：", "Online Beta 候选｜界面可访问，后端 / 核心编辑链路待在线复核；采用 BYO Key 使用边界。"),
    ])
