...
def render(output: Path) -> None:
    ...
    r.project("Service Agent", "RAG 客服与知识飞轮", [("目标：", "使用 LangGraph 8 节点 11 边工作流编排检索、风险分流、生成、质量检查与人工接管，完成 Web/API 端到端 MVP。"), ("验证：", "589 tests 全量回归（2026-07-28，非准确率）；固定 90 样本保留为静态 runner 审计输入。"), ("评测边界：", "现有 runner 未验证 expected_route、实际澄清节点或生成输出，因此不发布路由准确率、回答正确率或禁止承诺结果。"), ("公网状态：", "Controlled Demo｜静态 B1/B2/B3 演示可用，实时后端修复中；未接通时安全转人工。")])
