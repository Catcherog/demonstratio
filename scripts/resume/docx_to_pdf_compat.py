...
def render(output: Path) -> None:
    ...
    r.project("光砚 Lumen", "AI 图像编辑工作台", [("产品：", "将多模型图像生成与编辑能力组织为含任务状态、版本与失败恢复的工作台，覆盖图片生成、编辑与多格式导出。"), ("工程协作：", "Provider 抽象，以及项目 / 任务 / 结果持久化与恢复。"), ("本地验证：", "_id 更新缺陷已修复；本地 client/server 回归与 build 通过。"), ("状态与边界：", "Controlled Demo｜界面可访问，后端健康 / 核心编辑链路待恢复；采用 BYO Key 使用边界。")])
